...
def load_layers(model: AutoModelForCausalLM, layer_names: List[str], new_state: List[int], sparse_weights_path: str):
    ...
    for layer_name, new_level, old_level in zip(layer_names, new_state, model.state):
        ...
        if new_level != old_level:
            layer = model.get_submodule(layer_name)
            if hasattr(layer, 'weight'):  # 检查是否有weight属性
                weight_path = os.path.join(sparse_weights_path, layer_name, f"{new_level}.pth")
                layer.weight.data = torch.load(weight_path, map_location=layer.weight.device).to(layer.weight.dtype)
            else:
                print("layer_information:", layer)
    # Update model state
