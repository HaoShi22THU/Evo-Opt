...
# def compute_clip(model, clip_model, clip_preprocess, prompt, figure):
...
def compute_fitness(model, prompt, data, fitness_fn, invert_fitness, target_logits: Optional[torch.Tensor] = None) -> float:
    # 定义一个变量sign，默认为1
    sign = 1
    ...
    if fitness_fn == "ppl":
        return sign * compute_perplexity(model, prompt, data)
...
def selection(
    ...
    for candidate in candidates:
        ...
        fitness = compute_fitness(model, prompt, test_data, fitness_fn, invert_fitness)
        fitnesses.append(fitness)
...
def parse_args():
    ...
    parser.add_argument("--sparsity", type=float, default=0.125, help="Fraction of layers to drop.")
    # Logging params
    ...
    parser.add_argument("--offspring", type=int, default=32, help="Number of offspring generated in each generation")
