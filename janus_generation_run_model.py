...
def load_drop_config():
    with open("/mnt/temp/hshi/EvoPress/EvoPress/layer_drop_base.txt", "r") as f:
        lines = f.readlines()
    # 读取每一行的内容
...
def main():
    ...
    model1 = AutoModelForCausalLM.from_pretrained(model_name_or_path, trust_remote_code=True)

    model = model.to(torch.bfloat16).cuda().eval()
    model1 = model1.to(torch.bfloat16).cuda().eval()
